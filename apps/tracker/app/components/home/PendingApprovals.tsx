    ...
    <Card className="shadow-sm border-amber-200 bg-amber-50/50 mb-6">
      ...
      <CardContent className="grid gap-4">
        {approvals.map((entry) => {
          ...
          return (
            <div key={entry.id} className="bg-white p-4 rounded-lg border border-amber-200 flex items-center justify-between">
              <div>
                <p className="font-medium text-slate-800">{entry.todoTitle}</p>
                <p className="text-sm text-slate-500">{entry.projectName}</p>
              ...
              <Form method="post" className="flex items-center gap-3">
                ...
                <div className="flex flex-col gap-1 w-24">
                  <Label htmlFor={`duration-${entry.id}`} className="text-xs text-slate-500">Hours</Label>
                  <Input 
                ...
                <Button type="submit" size="sm" className="bg-amber-600 hover:bg-amber-700 mt-5">
                  Approve & Sync
                </Button>
              </Form>
            </div>
